(() => {
  ...
  const categoryLabels = { residential: "住宅", commercial: "商业", office: "办公" };
  ...
  const projects = {
    ...
    "office-finance-city": { category:"office", name:"金融城智汇中心", location:"高新区 · 交子大道", price:"320万 - 1280万", area:"120 - 520㎡", tags:["OFFICE","精装修"], image:"https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?auto=format&fit=crop&w=1000&q=80" },
        ...
        <article class="estate-card">
          <a class="estate-card__link" href="${getDetailUrl(id, project)}" aria-label="查看${project.name}详情">
            <div class="estate-card__media">
              ...
              <span class="estate-card__badge">${categoryLabels[project.category]}</span>
            </div>
            <div class="estate-card__body">
              <h2>${project.name}</h2>
              <p class="estate-card__location">${project.location}</p>
              <strong class="estate-card__price">¥${project.price}</strong>
              <div class="estate-card__tags">${project.tags.map((tag) => `<span>${tag}</span>`).join("")}</div>
              <div class="estate-card__meta">
                <div><span>面积区间</span><strong>${project.area}</strong></div>
                <div><span>销售状态</span><strong>在售</strong></div>
              </div>
            </div>
          </a>
